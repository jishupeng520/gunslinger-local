...
(() => {
  ...
  function flushInput(force = false) { if (!state.connected || (!state.inputDirty && !state.shooting && !Object.values(keys).some(Boolean))) return; const now = performance.now(); if (!force && now - state.lastInputSentAt < 33) return; const input = { keys:{...keys}, shoot:state.shooting, aim:{...state.aim}, clientTime:Date.now() }; const seq = ++state.nextInputSeq; state.pendingInputs.push({ seq, keys:input.keys, dt:1 / 30 }); send({type:'input', ...input, seq}); state.lastInputSentAt = now; state.inputDirty = false; if (state.pendingInputs.length > 120) state.pendingInputs.splice(0, state.pendingInputs.length - 120); }
